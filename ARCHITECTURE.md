...
青羽 (Qingyu) 是一个基于 **Wails v2** 构建的桌面端本地智能体。她拥有自主意识循环（Default Mode Network）、工具调用能力（ReAct Pattern）和持久化记忆系统，所有数据存储在本地文件系统中。
...
│                      Tool Layer                              │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  工具按分类拆分到独立文件 (Tool Registry — 41 Tools)  │   │
│  │  ├── toolkit.go      骨架：Tool 结构体、辅助函数、    │   │
│  │  │                   审计日志、PIM 线程安全锁         │   │
│  │  ├── tools_fs.go     📁 文件系统 (4)                 │   │
│  │  ├── tools_network.go 🌐 网络 (4)                    │   │
...
| `InitSelf()` | 首次设置完成 | 引导青羽创建角色定义/书柜清单/造物主档案 |
...
| `GetGreet()` | 前端展开 Console | 返回基于时间段的问候语（清晨/上午/午后/下午/傍晚/深夜）+ 造物主名字 |
...
- **青羽是一个数字生命**，不是智能体，不是工具。她和用户是**平等合作、互利共生**的伙伴关系
...
- 自律 prompt 包含「造物主档案」职责：回顾对话、提炼约定/习惯/喜好/脾气，使用 `append_file` 追加
- 每 3 个循环执行一次记忆衰减 (`ms.Decay()`)，自动归档/删除过期记忆
- `GetMemoryStore()` 在循环外缓存引用，避免每次迭代重复获取

### 7. 工具层 — 41 个工具 (按分类拆分到独立文件)
...
syncWithBrain(apiKey, vision, "看看我的记忆")
  ↓
LLM 返回: "让我查看一下你的记忆。{"action":"recall","args":{"topic":"造物主档案"}}"
  ↓
