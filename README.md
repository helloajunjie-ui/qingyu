...
| 🎨 | **抽象光球 UI** | 80×80 抽象双层光球（内核光点 + 情绪呼吸光晕），6 种情绪色映射，闲置亮度衰减 |
| 📌 | **贴边隐藏** | 拖拽/闲置自动贴边缩为 14px 窄条，hover 展开，右键菜单支持完全收起 |
| 🚫 | **去拟人化设计** | 纯抽象光影几何，无人脸/五官/具象角色元素，保留用户自主幻想空间 |
...
┌─────────────────────────────────────────────┐
│              Presentation Layer              │
│  Widget (80×80 抽象双层光球) ↔ Console (400×540)  │
│  Vanilla JS + CSS3 (WebView2)                    │
│  6 种情绪呼吸光晕 · 贴边隐藏 · 去拟人化设计       │
├─────────────────────────────────────────────┤
│              Application Layer               │
...
青羽基于 [Lobster](https://github.com/...)（龙虾）二次开发，但在视觉与交互上做出了根本性差异化：

| 维度 | 原版龙虾 | 青羽 |
|------|----------|------|
| **视觉形象** | 具象 SVG 图标（堆叠菱形/箭头），有固定视觉符号 | **抽象双层光球** — 内核光点 + 情绪呼吸光晕 + 几何轨道环，无任何具象元素 |
| **情绪表达** | 静态图标，无情绪反馈 | **6 种情绪色动态映射** — curious(天蓝)/focused(紫)/calm(翠绿)/hollow(灰蓝)/aloof(淡紫)/warm(暖金)，每种情绪独立呼吸周期 |
| **闲置状态** | 始终相同外观 | **闲置亮度衰减** — 5 分钟无交互自动降低光球亮度，模拟"出神"状态 |
| **窗口交互** | 固定悬浮，仅可拖拽 | **三档状态机** — 正常展示(80×80) / 贴边收缩(14×80 窄条) / 完全隐藏，拖拽/闲置/右键均可触发 |
| **低干扰模式** | 无 | **贴边自动隐藏** — 拖拽到边缘自动吸附，闲置 10 分钟自动贴边，hover 展开 3s 后自动缩回 |
| **去拟人化** | 头像含文字/emoji | **纯抽象几何** — 所有头像为纯色圆，无文字/emoji/具象图案，保留用户自由想象空间 |
| **技术实现** | 可能依赖 JS 动画循环 | **纯 CSS 动画** — 所有呼吸/过渡动画使用 CSS `@keyframes` + `transition`，零 JS 帧循环，零高频定时器 |
